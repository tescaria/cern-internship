...
def preprocess_data():
    reg_file_path = "/eos/user/t/tcostaes/traccc_outputs/roi/lxplus.csv"
    data_reg = np.genfromtxt(reg_file_path, delimiter=',', names=True, dtype=None, encoding='utf-8')

    full_file_path = "/eos/user/t/tcostaes/traccc_outputs/full/lxplus.csv"
    data_full = np.genfromtxt(full_file_path, delimiter=',', names=True, dtype=None, encoding='utf-8')

    superroi_file_path = "/eos/user/t/tcostaes/traccc_outputs/superroi/lxplus.csv"
...
superroi_dir = "/eos/user/t/tcostaes/traccc_outputs/roiInputJets_nodup"
...
def main():
    ...
    avg_hits_roi, _ = dataset_hits(roi_dir)
    avg_hits_full, _ = dataset_hits(full_dir)
    avg_hits_superroi, _ = dataset_hits(superroi_dir)
    ...
    x_superroi, y_superroi, yerr_superroi = aggregate_by_threads(data_superroi, avg_hits_superroi)
    ...
    tl.SetNDC()
    tl.SetTextSize(22)
    tl.DrawLatex(0.55, 0.20, "#bf{NVIDIA Tesla T4 GPU}")
    tl.SetTextSize(20)
    tl.DrawLatex(0.55, 0.16, "#sqrt{s} = 14 TeV, <#mu> = 200, t#bar{t}")
    tl.DrawLatex(0.55, 0.12, "ITk Layout: 03-00-01")

    # Legend
    leg = root.TLegend(0.15, 0.75, 0.43, 0.93)
    ...
    c.SaveAs("/eos/user/t/tcostaes/traccc_outputs/plots/throughput/hits/throughput_3datasets_lxplus_ratios.pdf")
